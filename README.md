...
- 淘汰赛使用 FIFA 公布的 73-104 号固定赛程树；第三名席位按候选组规则分配，平局后进入点球。
...
- 已新增 `data/squads_2026.csv`，从公开 squad tracker 导入 31 队、806 名球员的 26 人名单行；平均年龄和俱乐部分布已按球员行聚合，球员身价由球队总身价按角色/年龄/联赛/国家队资历分配，伤病仍保留待补字段。
...
当前版本是探索型概率工具。淘汰赛路径已接入 FIFA 固定 match-number bracket；第三名席位目前用候选组匹配算法分配，尚未展开 FIFA 对全部第三名组合的完整 495 种映射表。
